...
public final class SyFontAWTTest
{
  private SyFontDirectoryType<SyFontAWT> fonts;
  private SyFontAWT font;
  ...
  public void testLayout()
    throws Exception
  {
    try (var s = SyFontAWTTest.class.getResourceAsStream(
      "/com/io7m/jsycamore/tests/arctic.txt")) {
      final var text = new String(s.readAllBytes(), StandardCharsets.UTF_8);

      final var r =
        new ArrayList<>(this.font.textLayoutMultiple(List.of(text), 320));
  ...
  private static void check(
    final List<SyTextSectionLineType> lines,
    final String expected)
  {
    assertEquals(expected, lines.remove(0).text());
  }
}
