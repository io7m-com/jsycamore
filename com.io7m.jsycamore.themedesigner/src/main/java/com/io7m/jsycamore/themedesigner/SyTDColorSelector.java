...
final class SyTDColorSelector extends JPanel
{
  ...
  private Vector3D color_vector;
  private Color color;
  private float[] color_data;
  private List<Runnable> listeners;

  SyTDColorSelector(final Vector3D initial_color)
  {
    this.listeners = new ArrayList<>(1);

    this.color_data = new float[4];
    this.color_vector = Vectors3D.zero();
    ...
    this.select.addActionListener(e -> {
      final Color selected_color = JColorChooser.showDialog(
        ...
        "Select color_vector",
        new Color(
          (float) this.color_vector.x(),
          (float) this.color_vector.y(),
          (float) this.color_vector.z()));
    ...
    this.updateColor(new Color(
      (float) initial_color.x(),
      (float) initial_color.y(),
      (float) initial_color.z()));
  }
